...
Приложение шахматная доска перенесено из Lazarus в RAD Studio 10.4.1 с полным сохранением функциональности. Это открывает широчайший диапазон платформ для сборки Windows, Linux, MacOS, iOS, Android и др.
Надеюсь мое приложение хоть комуто пригодится.
