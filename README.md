...
# ascii-chess 1967
 Движок подключен к интерфейсу. Ссылка на исполняемый файл project1.exe в папке https://github.com/kirill7785/Chess/tree/main/ascii-chess%201967. Исполняемый файл project1.exe нужно запускать. При этом в одной папке с файлом project1.exe должна лежать chess.dll и debut_book.txt.  Движок реализует только два полухода без форсированого варианта, пропускает детский мат, но сопротивляется. Её приятно обыгрывать совсем слабому любителю шахмат.
 ...
 # Известные проблемы
1. С рокировкой для чёрных внутри dll не всё впорядке. Надо искать баг.
2. Из за высокой нагрузки при отрисовке доски с фигурами в Lazarus 2.2.4 приложение не закрывается  после File->Close или после нажатия на красный крестик. Закрыть приложение можно сделав последовательно File->Close, нажатие на красный крестик, нажатие свернуть форму и форма закроется. Чтобы можно было выбрать опонента на дочерней форме отрисовщик доски с фигурами приостанавливается на время выбора опонента.
