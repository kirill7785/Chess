...
# ascii-chess 1967
 Движок подключен к интерфейсу. Ссылка на исполняемый файл https://github.com/kirill7785/Chess/tree/main/ascii-chess%201967 файл project1.exe. project1.exe нужно запускать. При этом в одной папке с project1.exe должна лежать chess.dll и debut_book.txt.  Только два полухода без форсированого варианта, пропускает детский мат, но сопротивляется. Её приятно обыгрывать совсем слабому любителю шахмат.
 # Известные проблемы
1. С рокировкой для чёрных внутри dll не всё впорядке. Надо искать баг.
2. Почему то в Lazarus 2.2.4 приложение не закрывается  после File->Close или после нажатия на красный крестик. А Group Box на форме выбор опонента тупит при нажатии.
# Системные требования
Программа работает под ОС Windows.
# Используемое ПО
движок собран в Visual Stdio C++ Comunity. Графический интерфейс в Lazarus 2.2.4 Pascal.
