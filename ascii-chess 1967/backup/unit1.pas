...
// дочерние формы с настройками тоже не тормозят.    6165 строк.
...
var
  ...
  // Шахматный движок написан на С++ и оформлен в виде dll библиотеки:
  procedure analiticChess(cb : array of Integer;  clr : Boolean;
    move1 : array of Integer;  O_O : array of Boolean); cdecl; external 'chess.dll' name 'analiticChess';
                            ...
                            begin
                               ...
                               begin
                                  ...
                                  begin
                                     ...
                                     if (FormGamemode.RadioGroupOpponent.ItemIndex=1) then
                                     begin

                                        for i6:=0 to 63 do cb[i6]:=0;
                                        for i6:=0 to 15 do
                                        begin
                                           if (arrw[i6].fig<>cemptyfig) then
                                           begin
                                              case arrw[i6].fig of
                                               cpawn  : begin
                                                           i5:=(arrw[i6].i-1)*8+(arrw[i6].j-1);
                                                           cb[i5]:=6;
                                                        end;
                                               crook  : begin
                                                           i5:=(arrw[i6].i-1)*8+(arrw[i6].j-1);
                                        ...
                                        for i6:=0 to 15 do
                                        begin
                                           if (arrb[i6].fig<>cemptyfig) then
                                           begin
                                              case arrb[i6].fig of
                                               cpawn  : begin
                                                           i5:=(arrb[i6].i-1)*8+(arrb[i6].j-1);
                                                           cb[i5]:=12;
                                                        end;
                                               crook  : begin
                                                           i5:=(arrb[i6].i-1)*8+(arrb[i6].j-1);
                                        ...
                                        for i6:=0 to 3 do
                                        begin
                                            O_Oexp[i6]:=false;  // По умолчанию рокировки не было.
                                        end;
                                        ...
                                        move1exp[4]:= white_previos_move.from.j-1; // предыдущий ход противника (обычно это ход белых который был перед будущим ответом компа) откуда x [0..7]
                                        move1exp[5]:= white_previos_move.from.i-1; // предыдущий ход противника (обычно это ход белых который был перед будущим ответом компа) откуда y [0..7]
                                        move1exp[6]:= white_previos_move.to_.j-1; // предыдущий ход противника (обычно это ход белых который был перед будущим ответом компа) куда x [0..7]
                                        move1exp[7]:= white_previos_move.to_.i-1; // предыдущий ход противника (обычно это ход белых который был перед будущим ответом компа) куда y [0..7]
                                        ...
                                        // была ли рокировка белых в короткую сторону
                                        if (did_the_white_right_rook_move or  did_the_white_king_move) then O_Oexp[0]:=true;
                                         // была ли рокировка белых в длинную сторону
                                        if (did_the_white_left_rook_move or  did_the_white_king_move) then O_Oexp[1]:=true;
                                         // была ли рокировка чёрных в короткую сторону
                                        if (did_the_black_right_rook_move or  did_the_black_king_move) then O_Oexp[2]:=true;
                                         // была ли рокировка чёрных в длинную сторону
                                        if (did_the_black_left_rook_move or  did_the_black_king_move) then O_Oexp[3]:=true;
                                        ...
                                        i7:=1+move1exp[1]; // откуда y
                                        j8:=1+move1exp[2]; // куда x
                                        i8:=1+move1exp[3]; // куда y


                                       for j_5:=0 to High(arrb) do
                                       begin
                                          if ((arrb[j_5].fig<>cemptyfig) and (arrb[j_5].i=i7) and (arrb[j_5].j=j7)) then
                                          begin
                                         // куда
                                   if ((WhotisPole(i8,j8)<>cblack){and(vacantmove[i8,j8])}) then
                                   begin
                                     ...
                                     begin
                                      ...
                                      black_previos_move.from.i:=arrb[j_5].i;
                                      black_previos_move.from.j:=arrb[j_5].j;
                                      black_previos_move.to_.i:=i8;
                                      black_previos_move.to_.j:=j8;
                                     ...
                                     did_the_black_king_move1:=did_the_black_king_move;
                                      ...
                                      if (arrb[j_5].fig=crook) and (arrb[j_5].i=1)and(arrb[j_5].j=8) then
                                     did_the_black_right_rook_move:=true;
                                     if (arrb[j_5].fig=crook) and (arrb[j_5].i=1)and(arrb[j_5].j=1) then
                                     did_the_black_left_rook_move:=true;
                                     if (arrb[j_5].fig=cking) and (arrb[j_5].i=1)and(arrb[j_5].j=5) then
                                     ...
                                     beat:=false;
                                      ...
                                      arrb[j_5].j:=j8;
                                       // Превращение пешки в королеву.
                                     if (i8=8) and (arrb[j_5].fig=cpawn) then arrb[j_5].fig:=cqueen;
                                     black_previos_move.fig:=arrb[j_5].fig;
